...
assessmentButton.addEventListener( //イベント検知設定の追加
 'click',
    // ボタンが押されたときの処理
() => { // 無名関数でアロー関数、イベント検知したら実行
      const userName = userNameInput.value; // 入力欄（input)からの取得
       if (userName.length === 0){ // 入力がからだったら
        return;
       }

        // TODO 診断結果表示エリアの作成
        resultDivision.innerText = ''; //divタグをから文字で上書きすることで空白にしている
        //TODO ツイートエリアの作成
        ...
        // X投稿ボタンの作成
        tweetDivision.innerText = ''; //divタグをから文字で上書きすることで空白にしている
        const anchor = document.createElement('a'); // aタグを作成
        const hrefValue = 'https://twitter.com/intent/tweet?button_hashtag='+ encodeURIComponent('あなたのいいところ') + '&ref_src=twsrc%5Etfw' ;
            
        anchor.setAttribute('href', hrefValue); // aタグのhref属性に値を設定
        anchor.setAttribute('class','twitter-hashtag-button');
        anchor.setAttribute('data-text',result);
        anchor.innerText = 'Tweet #あなたのいいところ'; // aタグのテキストを設定
